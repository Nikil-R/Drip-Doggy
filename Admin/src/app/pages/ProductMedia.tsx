  ...
  return (
    <div className="space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-[#030213] uppercase tracking-widest">Product Media</h1>
          <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider mt-0.5">
            Drip Doggy product images &amp; assets
          </p>
        </div>
        <button onClick={() => setShowAdd(true)} className="bg-[#030213] hover:bg-neutral-800 text-white text-[9px] font-semibold tracking-widest px-4 py-2 uppercase flex items-center gap-1.5 cursor-pointer rounded-none border-none">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-card border border-neutral-200/80 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-lg font-bold text-[#030213]">{totalImages}</p>
            <p className="text-[7px] text-neutral-400 font-semibold uppercase tracking-wider">Total Images</p>
          </div>
        </div>
        <div className="bg-card border border-neutral-200/80 p-4 flex items-center gap-3">
          <Star className="w-5 h-5 text-amber-600 shrink-0" />
          <div>
            <p className="text-lg font-bold text-amber-700">{primaryImages}</p>
            <p className="text-[7px] text-neutral-400 font-semibold uppercase tracking-wider">Primary Images</p>
          </div>
        </div>
        <div className="bg-card border border-neutral-200/80 p-4 flex items-center gap-3">
          <Layers className="w-5 h-5 text-neutral-400 shrink-0" />
          <div>
            <p className="text-lg font-bold text-[#030213]">{productsWithMedia}</p>
            <p className="text-[7px] text-neutral-400 font-semibold uppercase tracking-wider">Products w/ Media</p>
          </div>
        </div>
        <div className="bg-card border border-neutral-200/80 p-4 flex items-center gap-3">
          <Eye className="w-5 h-5 text-neutral-400 shrink-0" />
          <div>
            <p className="text-lg font-bold text-[#030213]">{allProducts.length}</p>
            <p className="text-[7px] text-neutral-400 font-semibold uppercase tracking-wider">Total Products</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 bg-card border border-neutral-200/80 p-4">
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" placeholder="Search by product or alt text..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full border border-neutral-200/80 pl-8 pr-3 py-2 text-[9px] font-semibold uppercase tracking-widest focus:outline-none focus:border-[#030213] placeholder-neutral-300" />
        </div>
        <select value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)}
          className="border border-neutral-200/80 px-3 py-2 text-[9px] font-bold uppercase focus:outline-none focus:border-[#030213] bg-card">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map(m => (
          <div key={m.id} className="bg-card border border-neutral-200/80 group relative">
            <div className="aspect-square bg-neutral-100 overflow-hidden relative">
              <img src={m.url} alt={m.alt} className="w-full h-full object-cover" />
              {m.isPrimary && (
                <div className="absolute top-2 left-2 bg-[#030213] text-white text-[6px] font-semibold tracking-widest px-1.5 py-0.5 uppercase">
                  PRIMARY
                </div>
              )}
            </div>
            <div className="p-3">
              <p className="text-[8px] font-bold text-[#030213] uppercase tracking-wide truncate">{m.productName}</p>
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-neutral-100">
                {m.isPrimary ? (
                  <span className="text-[7px] font-semibold text-green-700 bg-green-50 px-2 py-0.5 uppercase">Primary</span>
                ) : (
                  <button onClick={() => setPrimary(m.id)} className="text-[7px] font-semibold text-neutral-400 hover:text-[#030213] cursor-pointer bg-transparent border-none uppercase">Set Primary</button>
                )}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowAdd(false)}>
          <div className="bg-card border border-neutral-200 w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-sm font-bold text-[#030213] uppercase tracking-widest">Add Media</h2>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[8px] font-bold tracking-wider text-neutral-500 uppercase mb-1 block">Product</label>
                <select value={newProduct} onChange={e => setNewProduct(e.target.value)}
                  className="w-full border border-neutral-200/80 px-3 py-2 text-[9px] font-bold uppercase focus:outline-none focus:border-[#030213] bg-card">
              ...
              <div>
                <label className="text-[8px] font-bold tracking-wider text-neutral-500 uppercase mb-1 block">Alt Text</label>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-neutral-100">
              <button onClick={() => setShowAdd(false)} className="border border-neutral-200 hover:border-[#030213] text-neutral-500 text-[9px] font-semibold tracking-widest px-4 py-2 uppercase bg-card cursor-pointer">Cancel</button>
              <button onClick={addMedia} className="bg-[#030213] hover:bg-neutral-800 text-white text-[9px] font-semibold tracking-widest px-4 py-2 uppercase cursor-pointer border-none">Add</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setDeleteId(null)}>
          <div className="bg-card border border-neutral-200 p-6 max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-sm font-bold text-[#030213] uppercase tracking-widest mb-2">Delete Media?</h3>
            <p className="text-[9px] text-neutral-500 mb-4">This action cannot be undone.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setDeleteId(null)} className="border border-neutral-200 hover:border-[#030213] text-neutral-500 text-[9px] font-semibold tracking-widest px-4 py-2 uppercase bg-card cursor-pointer">Cancel</button>
              <button onClick={() => { setMedia(media.filter(m => m.id !== deleteId)); setDeleteId(null); }} className="bg-[#b2533e] hover:bg-red-800 text-white text-[9px] font-semibold tracking-widest px-4 py-2 uppercase cursor-pointer border-none">Delete</button>
            </div>
          </div>
        </div>
      )}
