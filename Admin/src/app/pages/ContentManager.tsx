...
const sections = [
  ...
  { name: "Collection Story", icon: FileText, status: "Published", lastEdited: "3 days ago", desc: "Brand narrative section", route: "collection-story", color: "from-[#717182] to-[#4a4a5a]" },
...
export function ContentPage() {
  return (
    <div className="space-y-8 font-sans">
      ...
      {/* ── Header ─────────────────────────────────────────────────── */}
      <div>
        <h1 className="text-xl font-black text-[#030213] uppercase tracking-widest">Content Manager</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white border border-neutral-200/80 p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-black text-[#030213]">{stat.value}</p>
              <p className="text-[7px] text-neutral-400 font-extrabold uppercase tracking-wider">{stat.label}</p>
      ...
      <div className="flex items-center justify-between bg-white border border-neutral-200/80 p-4">
        ...
        <div className="flex gap-1.5">
          <button className="px-3 py-1.5 bg-[#030213] text-white text-[7px] font-extrabold tracking-widest uppercase border-none cursor-pointer">All</button>
          <button className="px-3 py-1.5 bg-transparent text-neutral-400 hover:text-[#030213] text-[7px] font-extrabold tracking-widest uppercase border-none cursor-pointer">Active</button>
          <button className="px-3 py-1.5 bg-transparent text-neutral-400 hover:text-[#030213] text-[7px] font-extrabold tracking-widest uppercase border-none cursor-pointer">Draft</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {sections.map((section) => (
          <div
            key={section.name}
            className="bg-white border border-neutral-200/80 hover:shadow-sm transition-all group cursor-pointer"
          >
            {/* Colored top bar */}
            ...
            <div className="p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="w-9 h-9 bg-[#faf8f5] border border-neutral-200 flex items-center justify-center group-hover:border-neutral-400 transition-colors">
              ...
              <h3 className="text-[10px] font-black text-[#030213] uppercase tracking-wide">{section.name}</h3>
      ...
      <div className="bg-white border border-neutral-200/80 p-6">
        <h2 className="text-[9px] font-black tracking-[0.2em] text-neutral-400 uppercase mb-4">Content Overview</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: "Pending Review", value: "3", sub: "Collection Story draft" },
            { label: "Next Scheduled", value: "SS26 Drop", sub: "Featured Products update" },
          ].map((item, i) => (
            <div key={i} className="border border-neutral-100 bg-[#faf8f5]/60 p-3">
              <p className="text-[7px] font-extrabold tracking-wider text-neutral-400 uppercase">{item.label}</p>
              <p className="text-[11px] font-black text-[#030213] mt-0.5">{item.value}</p>
