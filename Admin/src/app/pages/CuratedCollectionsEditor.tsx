  ...
  return (
    <div className="space-y-8 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-[#030213] uppercase tracking-widest">Curated Collections</h1>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={openAdd} className="bg-[#030213] hover:bg-neutral-800 text-white text-[9px] font-semibold tracking-widest px-4 py-2 uppercase flex items-center gap-1.5 cursor-pointer rounded-none border-none"><Plus className="w-3.5 h-3.5" /> Add Collection</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sorted.map((c, idx) => (
          <div key={c.id} className="bg-card border border-neutral-200/80 overflow-hidden">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
          <div className="bg-card border border-neutral-200 w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-sm font-bold text-[#030213] uppercase tracking-widest">{editItem ? "Edit Collection" : "Add Collection"}</h2>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-[8px] font-bold tracking-wider text-neutral-500 uppercase mb-1 block">Link (optional)</label>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-neutral-100">
              ...
              <button onClick={save} className="bg-[#030213] hover:bg-neutral-800 text-white text-[9px] font-semibold tracking-widest px-4 py-2 uppercase cursor-pointer rounded-none border-none">{editItem ? "Update" : "Create"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setDeleteId(null)}>
          <div className="bg-card border border-neutral-200 p-6 max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-sm font-bold text-[#030213] uppercase tracking-widest mb-2">Delete Collection?</h3>
