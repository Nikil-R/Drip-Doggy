  ...
  return (
    <div className="space-y-8 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-[#030213] uppercase tracking-widest">Footer Settings</h1>
      ...
      <div className="bg-white border border-neutral-200/80 p-6 space-y-4">
        <h3 className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Brand Info</h3>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="text-[8px] font-black tracking-wider text-neutral-500 uppercase mb-1 block">Copyright Text</label>
      ...
      {/* CTA Section */}
      <div className="bg-white border border-neutral-200/80 p-6 space-y-4">
        <h3 className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">CTA Section (Newsletter in Footer)</h3>
        ...
        <div>
          <label className="text-[8px] font-black tracking-wider text-neutral-500 uppercase mb-1 block">Chips (comma-separated)</label>
      ...
      {/* Link Groups */}
      <div className="bg-white border border-neutral-200/80 p-6 space-y-4">
        <h3 className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Link Groups</h3>
          ...
          <div key={gi} className="border border-neutral-100 p-4 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setConfigState({ ...config, linkGroups: config.linkGroups.filter((_, i) => i !== gi) })}
                className="text-[#b2533e] text-[7px] font-extrabold cursor-pointer bg-transparent border-none">REMOVE</button>
            </div>
            ...
            <button onClick={() => {
              ...
              setConfigState({ ...config, linkGroups: g });
            }} className="text-[7px] font-extrabold text-[#030213] hover:text-neutral-600 cursor-pointer bg-transparent border-none ml-4">+ Add Link</button>
          </div>
        ))}
        <button onClick={() => setConfigState({ ...config, linkGroups: [...config.linkGroups, { title: "", links: [] }] })}
          className="border border-dashed border-neutral-300 hover:border-[#030213] text-neutral-500 text-[9px] font-extrabold tracking-widest px-4 py-2 uppercase w-full cursor-pointer bg-transparent">+ Add Link Group</button>
      </div>

      {/* Social Links */}
      <div className="bg-white border border-neutral-200/80 p-6 space-y-4">
        <h3 className="text-[9px] font-black text-neutral-500 uppercase tracking-widest">Social Links</h3>
        {config.socialLinks.map((sl, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <button onClick={() => {
              ...
              setConfigState({ ...config, socialLinks: s });
            }} className={`text-[7px] font-extrabold tracking-widest px-2 py-1 cursor-pointer border-none ${sl.active ? "bg-green-50 text-green-700" : "bg-neutral-100 text-neutral-400"}`}>{sl.active ? "ON" : "OFF"}</button>
        ...
        <button onClick={() => setConfigState({ ...config, socialLinks: [...config.socialLinks, { platform: "", label: "", url: "", active: true }] })}
          className="text-[7px] font-extrabold text-[#030213] hover:text-neutral-600 cursor-pointer bg-transparent border-none">+ Add Social Link</button>
      </div>
