      ...
      <div className="max-w-4xl mx-auto px-6 py-12 flex-1">
        ...
        {orders.length === 0 ? (
          <div className="bg-white border border-neutral-200/80 p-16 text-center">
            ...
            <Link to="/shop" className="inline-block bg-[#030213] hover:bg-neutral-800 text-white px-8 py-3 text-[10px] font-extrabold tracking-[0.2em] uppercase transition-colors">Start Shopping</Link>
          </div>
        ) : (
                  ...
                  {order.items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 px-6 py-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] text-neutral-500 font-bold uppercase tracking-wider mt-0.5">Size: {item.size} | Color: {item.color} | Qty: {item.quantity}</p>
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    {order.status !== "Delivered" ? (
                      <button onClick={() => alert(`Order ${order.id} cancellation submitted.`)}
                        className="bg-white border border-red-200 hover:bg-red-50 text-red-500 px-4 py-2 text-[8px] font-extrabold tracking-widest uppercase transition-all cursor-pointer">Cancel</button>
                    ) : (
                      <button onClick={() => alert(`Return initiated for order ${order.id}.`)}
                        className="bg-white border border-neutral-200 hover:border-[#030213] text-neutral-600 px-4 py-2 text-[8px] font-extrabold tracking-widest uppercase transition-all cursor-pointer">Return</button>
                    )}
                    <button onClick={() => printInvoice(order, profile)}
